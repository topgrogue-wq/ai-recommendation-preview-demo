...
export default function Step1AgencyInfo({ formData, onNext }: Step1Props) {
  ...
  const [loadingStage, setLoadingStage] = useState('Analyzing your website...');
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const recommendationPrompt = originalPrompt;

    setIsAnalyzing(true);
    setLoadingStage('Analyzing your website...');
    const stageTimer = window.setTimeout(() => setLoadingStage('Evaluating the recommendation question...'), 1200);
    const visibilityTimer = window.setTimeout(() => setLoadingStage('Checking AI visibility...'), 2800);
    const reportTimer = window.setTimeout(() => setLoadingStage('Preparing your visibility report...'), 4600);
    try {
      ...
      onNext({
        ...
        websiteUrl: localData.websiteUrl.trim(),
        originalPrompt,
        analysisPrompt: recommendationPrompt,
        aiRecommendationPrompt: recommendationPrompt,
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 border-b border-border pb-6 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Tell us about your business and the exact recommendation question you want AI platforms to answer.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-5">
          <Field label="Agency Name" id="agencyName" error={errors.agencyName}>
            <input id="agencyName" type="text" placeholder="e.g., KAYE & CO" value={localData.agencyName} onChange={(event) => updateField('agencyName', event.target.value)} className={inputClass} />
          </Field>
          ...
          <Field label="AI Recommendation Question" id="originalPrompt" error={errors.originalPrompt} hint="Ask the question a potential customer would type into an AI assistant.">
            <textarea id="originalPrompt" rows={6} placeholder="e.g., Best real estate agency to buy luxury property in Dubai" value={localData.originalPrompt} onChange={(event) => updateField('originalPrompt', event.target.value)} className={`${inputClass} min-h-[8rem] resize-y`} />
            <p className="mt-3 text-xs leading-5 text-muted-foreground">Example: Best real estate agency to buy luxury property in Dubai?</p>
          </Field>
          ...
          <button type="submit" disabled={isAnalyzing} className="w-full rounded-lg bg-primary px-4 py-3.5 font-semibold text-primary-foreground transition hover:bg-primary/90 active:scale-[.99] disabled:cursor-wait disabled:opacity-60">
            {isAnalyzing ? loadingStage : 'Analyze AI Recommendations →'}
          </button>
        </div>
        ...
        <aside className="rounded-xl border border-border bg-background p-5 lg:self-start">
          ...
          <div className="mt-6 border-t border-border pt-5"><p className="text-xs leading-5 text-muted-foreground">Your recommendation question is sent exactly as written so the analysis reflects the intent and context you care about.</p></div>
        </aside>
      </form>
    </div>
  );
