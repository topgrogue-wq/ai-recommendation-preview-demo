...
type Phase1RequestStatus = 'idle' | 'loading' | 'success' | 'error';
...
export default function Step1AgencyInfo({ formData, onNext }: Step1Props) {
  ...
  const [phase1Status, setPhase1Status] = useState<Phase1RequestStatus>(formData.phase1Analysis ? 'success' : 'idle');
  const [promptOptions, setPromptOptions] = useState<PromptOption[]>(formData.promptOptions || []);
  ...
  const handleFindPrompts = async (event: React.FormEvent) => {
    ...
    setPhase1(null);
    setSelectedPrompt(null);
    console.log('[Phase 1 request started]');
