...
export default function BrowserPreview({ agencyName, websiteUrl, variant = 'current' }: BrowserPreviewProps) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border bg-card shadow-sm ${improved ? 'border-primary/35 shadow-primary/5' : 'border-border'}`}>
      <div className="flex items-center gap-3 border-b border-border bg-muted px-4 py-3">
        ...
        <div className="min-w-0 flex-1 truncate rounded border border-border bg-background px-3 py-1.5 text-center text-xs text-muted-foreground">
          {address}
        </div>
      </div>
      ...
      <div className="bg-background">
        <div className="border-b border-border px-5 py-4 md:px-7">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="hidden items-center gap-4 text-xs text-muted-foreground sm:flex">
              ...
              <span>About</span>
              <span className="rounded border border-border px-2.5 py-1.5 text-foreground">Contact</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative grid gap-6 px-5 py-8 md:min-h-[345px] md:grid-cols-[1fr_0.9fr] md:px-7 md:py-10">
          <div className="flex flex-col justify-center">
            <p className="mb-3 text-[11px] font-medium uppercase tracking-[0.16em] text-primary">
              {improved ? 'Austin luxury real estate · Local experts' : 'Austin luxury real estate'}
            </p>
            <h2 className="max-w-md text-2xl font-semibold leading-tight tracking-tight text-foreground md:text-3xl">
              {improved ? 'Austin luxury homes, guided by local expertise.' : 'Find a home that feels like Austin.'}
            </h2>
            <p className="mt-3 max-w-md text-xs leading-5 text-muted-foreground md:text-sm">
              {improved
                ? 'Specializing in luxury homes across West Lake Hills, Barton Creek, and Central Austin.'
                : 'Explore thoughtfully selected homes across the city\'s most sought-after neighborhoods.'}
            </p>
            <div className="mt-6 flex items-center gap-2">
              <div className="rounded bg-foreground px-3.5 py-2 text-[11px] font-medium text-background">
                {improved ? 'Explore Austin homes' : 'Explore homes'}
              </div>
              <div className="rounded border border-border px-3.5 py-2 text-[11px] font-medium text-foreground">
                {improved ? 'Talk with a local advisor' : 'Meet our team'}
              </div>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2.5" aria-label="Property imagery preview">
            ...
            <div className="col-span-2 flex items-center justify-between rounded border border-border bg-card px-3.5 py-3">
              <div>
                <p className="text-[11px] font-medium text-foreground">{improved ? 'Austin properties by neighborhood' : 'Curated Austin homes'}</p>
                <p className="mt-1 text-[10px] text-muted-foreground">West Lake Hills · Barton Creek · Zilker</p>
              </div>
              <span className="text-[10px] text-muted-foreground">View collection</span>
            </div>
          </div>
        ...
        <div className="border-t border-border px-5 py-2.5 text-center text-[10px] text-muted-foreground">
          {improved ? 'Preview of one AI-friendly improvement' : 'Preview reconstructed from publicly visible page content'}
        </div>
      </div>
    </div>
  );
