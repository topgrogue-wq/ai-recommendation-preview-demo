...
export default function Step2WebsitePreview({ formData, onNext, onBack }: Step2Props) {
  ...
  return (
    <div className="space-y-6 pb-4">
      ...
      <div className="flex gap-3 border-t border-border pt-5"><button onClick={onBack} className="flex items-center gap-2 rounded-md bg-secondary px-4 py-3 font-medium text-secondary-foreground transition hover:bg-secondary/90"><ChevronLeft size={18} aria-hidden="true" />Back</button><button onClick={() => onNext(formData)} className="flex-1 rounded-md bg-primary px-4 py-3 font-medium text-primary-foreground transition hover:bg-primary/90">Continue to Website Analysis →</button></div>
    </div>
  );
}
